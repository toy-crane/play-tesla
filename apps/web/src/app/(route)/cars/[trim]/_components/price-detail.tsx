...
const getSubsidy = async ({
  ...
  if (WheelError || wheel === null) {
    throw new Error(WheelError?.message || "Wheel not found");
  }

  const wheelId = wheel.id;

  const { data: wheelSubsidy, error: wheelSubsidyError } = await supabase
    ...
    .maybeSingle();
  if (wheelSubsidyError) {
    return {
      error: wheelSubsidyError,
    };
  }
  ...
  const { data: generalSubsidy, error: generalSubsidyError } = await supabase
    .from("subsidies")
    .select("*")
    .eq("region_code", regionCode)
    .eq("trim_id", trimId)
  ...
  if (generalSubsidyError) {
    return {
      error: generalSubsidyError,
    };
  }
...
async function PriceDetail({
  ...
  const [regionSubsidyResponse, subsidyResponse] = await Promise.all([
    ...
    getSubsidy({
      regionCode,
      trimId: trimDetail.id,
      wheelCode: option.wheel,
    }),
  ]);
